    ...
    public class Formation
    {
        public Formation(List<Pt> weak, List<Pt> strong)
        {
            ...
            BackfieldReceivers = Enumerable.Range(strong.Count + weak.Count, backfield)
                ...
                .Select(moveOffLine)
                .ToList();
            Qb = Position.Ineligible(Pt.O);
            Line = Enumerable.Range(5, 6).Select(makeIneligible).ToList();
        }

        public List<Position> Positions => EligibleReceivers.Concat(Line).ToList();
        ...
        public Position Qb { get; set; }
        public List<Position> Line { get; }
    }
}